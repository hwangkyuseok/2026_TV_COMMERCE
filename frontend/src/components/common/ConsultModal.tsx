...
// Modal focusable areas: "input" | "save" | "kakao" | "sms" | "close"
...
export default function ConsultModal({ product, onClose }: ConsultModalProps) {
  ...
  // Focus input element when area is "input"
  useEffect(() => {
    if (focusArea === "input") {
  ...
  const moveFocus = useCallback(
    (dir: "prev" | "next") => {
      ...
      if (dir === "next" && idx < FOCUS_ORDER.length - 1) {
        setFocusArea(FOCUS_ORDER[idx + 1]);
  ...
  const handleKeyDown = useCallback(
    (e: KeyboardEvent) => {
      if (focusArea === "input") {
        // Allow numeric input when focused on input
        if (e.key >= "0" && e.key <= "9") {
      ...
      switch (e.key) {
        ...
        case KEY_CODES.ARROW_LEFT:
          ...
          if (BOTTOM_BUTTONS.includes(focusArea as BottomBtn)) {
            ...
            if (idx > 0)
              setFocusArea(BOTTOM_BUTTONS[idx - 1] as FocusArea);
          }
          break;
        case KEY_CODES.ARROW_RIGHT:
          ...
          if (BOTTOM_BUTTONS.includes(focusArea as BottomBtn)) {
            const idx = BOTTOM_BUTTONS.indexOf(focusArea as BottomBtn);
            if (idx < BOTTOM_BUTTONS.length - 1)
              setFocusArea(BOTTOM_BUTTONS[idx + 1] as FocusArea);
          }
          break;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80">
      ...
      >
        {/* Title */}
        <p className="text-center text-white text-sm font-medium mb-4 leading-relaxed">
          입력하신 휴대폰 번호로
          <br />
          주문정보를 전달드립니다
        </p>
        ...
        <p className="text-center text-tv-muted text-xs mb-4 truncate px-2">
          {product.name}
        </p>
