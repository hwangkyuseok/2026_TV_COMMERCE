...
interface SidebarProps {
  menus: string[];
  activeIndex: number;       // Currently active (selected + focused) menu index
  isSectionFocused: boolean; // Whether the sidebar zone has keyboard focus
}
    ...
    >
      ...
      <div className="px-5 pt-5 pb-4 border-b border-white/[0.05] shrink-0">
        <div className="flex items-center gap-2.5">
          ...
          >
            <span className="text-black text-[10px] font-black leading-none">TV</span>
          </div>
          <div className="min-w-0">
            <p className="text-white text-[13px] font-bold tracking-wider leading-tight">
              Commerce
            </p>
            <p className="text-tv-muted text-[9px] tracking-[0.2em] mt-0.5">SMART TV</p>
          </div>
        </div>
      </div>
      ...
      <nav className="flex-1 flex flex-col px-2.5 py-4 gap-0.5">
        {menus.map((menu, idx) => {
          ...
          return (
            ...
            >
              {/* Left accent line for active (non-focused) item */}
              {isActive && !isFocused && (
                <span
